...
public class UDPManager {
	...
	public UDPRequest[] createUDPReqObj(List<ServerManager.CenterServerInfo> processList,
			FIFORequestQueueModel fifoRequestQueueModel) {
		UDPRequest[] requests;
		int counter = 0;
		...
		for (ServerManager.CenterServerInfo centerServerInfo : processList) {
			if (centerServerInfo.getPort() != myPort) {
				try {
					requests[counter] = new UDPRequest(centerServerInfo.getLocation(),
							centerServerInfo.getHostAddress(), centerServerInfo.getPort(), fifoRequestQueueModel);
				} catch (IOException e) {
					e.printStackTrace();
				}
	 ...
	 * @return
	 */
	public String sendBackUpProcessRequestFromController(List<ServerManager.CenterServerInfo> processList,
			FIFORequestQueueModel fifoRequestQueueModel) {
	...
	public String sendBackUpProcessRequests(FIFORequestQueueModel requestQueueModel) {
		return ServerManager.getInstance()
				.getCenterServer(serverLocation, ServerManager.getInstance().getMasterServerPort(serverLocation))
				.sendBackUpProcessRequestFromController(requestQueueModel);
	}
	...
	/*
	 * public String transferRecordsFromProcess(List<ServerManager.CenterServerInfo>
	 * processList, FIFORequestQueueModel fifoRequestQueueModel){
	 * 
	 * UDPRequest[] requests = createUDPReqObj(processList,fifoRequestQueueModel);
	 * for(UDPRequest req : requests){
	 * req.getCenterServer().transferRecord(managerId, recordId,
	 * remoteCenterServerName); } return "SUCCESS"; }
	 */
	...
	public String getRecordCounts(String managerId) {
		...
		for (Location location : Location.values()) {
			if (location == serverLocation) {
				...
				try {
					ServerManager.CenterServerInfo centerInfo = ServerManager.getInstance()
							.getMasterServerInfo(location);
					FIFORequestQueueModel reqObj = new FIFORequestQueueModel(RequestType.GET_RECORD_COUNT, managerId,
							location);
					requests[counter] = new UDPRequest(location, centerInfo.getHostAddress(), centerInfo.getPort(),
							reqObj);
		...
		serverLogger.log(Level.INFO, CMSLogMessages.RECORD_COUNT + " requested by manager: " + managerId,
	...
	public void transferRecord(String managerId, Record record, String remoteCenterServerName, char typeOfRec) {
		try {
			ServerManager.CenterServerInfo centerInfo = ServerManager.getInstance()
					.getMasterServerInfo(Location.valueOf(remoteCenterServerName));
			UDPRequest serverObject = null;
			if (typeOfRec == 'S') {
				FIFORequestQueueModel reqModel = new FIFORequestQueueModel(RequestType.CREATE_S_RECORD, record,
						managerId, Location.valueOf(remoteCenterServerName));
				serverObject = new UDPRequest(Location.valueOf(remoteCenterServerName), centerInfo.getHostAddress(),
						centerInfo.getPort(), reqModel);
				serverObject.start();
			} else if (typeOfRec == 'T') {
				FIFORequestQueueModel reqModel = new FIFORequestQueueModel(RequestType.CREATE_T_RECORD, record,
						managerId, Location.valueOf(remoteCenterServerName));
				serverObject = new UDPRequest(Location.valueOf(remoteCenterServerName), centerInfo.getHostAddress(),
						centerInfo.getPort(), reqModel);
				serverObject.start();
			...
			serverLogger.log(Level.INFO,
					String.format(CMSLogMessages.TRANSFER_RECORD_SUCCESS, record.getRecordId(), managerId));
		} catch (IOException e) {
			e.printStackTrace();
		} catch (InterruptedException e) {
			e.printStackTrace();
		...
		for (int i = 0; i < processIdList.size(); i++) {
			if (processIdList.get(i) != myPort && processIdList.get(i) > myPort) {
				try {
					...
					ServerManager.CenterServerInfo serverInfo = ServerManager.getInstance().getServerInfo(location,
							processIdList.get(i));
					if (serverInfo != null) {
						messageSent = true;
						UDPRequest udpRequest = new UDPRequest(serverInfo.getLocation(), serverInfo.getHostAddress(),
								serverInfo.getPort(), model);
						udpRequest.start();
					}
				} catch (IOException e) {
					e.printStackTrace();
