    ...
    public static void startAllServer() {
        ...
        for (int i = 0; i < hostPortArray.length; i++) {
            ...
            try {
                ...
                ServerManager.getInstance().addServer(Location.valueOf(hostPortArray[i][0]),Integer.parseInt(hostPortArray[i][1]), wsCenterServer.getCenterServerCenterImpl(),  hostPortArray[i][2]);
            } catch (IOException e) {
                e.printStackTrace();
            }catch (Exception ex){
